...
def test_add_people_to_group():
    ...
    response = client.post("/api/group/" + group_id + "/user/" + Student_id_list[0]["user_id"], headers=headers)
    assert response.status_code == 200 or response.status_code == 201
    response = client.get("/api/group/" + Course_id_list[0], headers=headers)
    assert response.status_code == 200
    groups = response.json().get("groups", [])
    ...
    response = client.delete("/api/group/" + group_id + "/user/" + Student_id_list[0]["user_id"], headers=headers)
    print("*" * 200)
    print(response.json())
    print("*" * 200)
    assert response.status_code == 200
